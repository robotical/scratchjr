...
export const { isiOS, isWebapp, isAndroid } = getPlatforms();
console.log("isiOS:", isiOS, "isWebapp:", isWebapp, "isAndroid:", isAndroid);
...
export function css_vh_ratio_adj(y) {
  const IW = WINDOW_INNER_WIDTH;
  const IH = WINDOW_INNER_HEIGHT;
  if (IW / IH > 1.2 && IW / IH <= 1.3)
    return (y * WINDOW_INNER_HEIGHT) / 105.0 + "px";
  if (IW / IH > 1.1 && IW / IH <= 1.2)
    return (y * WINDOW_INNER_HEIGHT) / 110.0 + "px";
  if (IW / IH > 1 && IW / IH <= 1.1)
    return (y * WINDOW_INNER_HEIGHT) / 115.0 + "px";
  if (IW / IH <= 1) return (y * WINDOW_INNER_HEIGHT) / 150.0 + "px";
  return (y * WINDOW_INNER_HEIGHT) / 100.0 + "px";
}

export function css_vw_ratio_adj(x) {
  const IW = WINDOW_INNER_WIDTH;
  const IH = WINDOW_INNER_HEIGHT;
  if (IW / IH > 1.2 && IW / IH <= 1.3)
    return (x * WINDOW_INNER_WIDTH) / 105.0 + "px";
  if (IW / IH > 1.1 && IW / IH <= 1.2)
    return (x * WINDOW_INNER_WIDTH) / 110.0 + "px";
  if (IW / IH > 1 && IW / IH <= 1.1)
    return (x * WINDOW_INNER_WIDTH) / 115.0 + "px";
  if (IW / IH <= 1) return (x * WINDOW_INNER_WIDTH) / 150.0 + "px";
  return (x * WINDOW_INNER_WIDTH) / 100.0 + "px";
}
