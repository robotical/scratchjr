...
export default class PublishedDataAnalyser {
    ...
    detectMovement(data) {
        return ShakeDetector.detectShake(data.LSM6DS.ax, data.LSM6DS.ay, data.LSM6DS.az, Date.now(),
            this.cogBlocks.onShake,
            this.cogBlocks.onMove
        );
    }

    detectRotation(data, isMoving) {
        RotationDetection.detectRotation(data,
            this.cogBlocks.onRotateClockwise,
            this.cogBlocks.onRotateCounterClockwise,
            isMoving
        );
    }
...
class TiltDetection {
    ...
    static detectTilt(data, { onTiltLeft, onTiltRight, onTiltForward, onTiltBackward }, systemVersion, isMoving = false) {
        if (isMoving) return;
        ...
        if (isVersionGreater_errorCatching(systemVersion, correctionCutOffVersion)) {
