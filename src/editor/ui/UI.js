...
export default class UI {
  ...
  static leftPanel(div) {
    ...
    martyConn.onclick = function (evt) {
  ...
  static setShowTrace(b) {
    Trace.hide(!b);
    gn("traceBtn").className = Trace.hidden
      ? "traceToggle off"
      : "traceToggle on";
  }
  ...
  static setMenuTextColor(t) {
    ...
    for (var i = 0; i < gn("textcolormenu").childElementCount; i++) {
      var mycolor = gn("textcolormenu").childNodes[i].childNodes[0]
        .childNodes[0].style.backgroundColor;
      if (c == mycolor) {
