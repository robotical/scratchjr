...
export default class ScratchJr {
  ...
  static displayStatus(type) {
    ...
    for (const id of ids) {
      if (gn(id)) {
        if (type == "block" && id == "blockspalette") {
          // blockspalette should be displayed as a grid
          // not as a block because we don't want it to wrap in smaller screens
          gn(id).style.display = "grid";
        } else {
          gn(id).style.display = type;
        }
      }
    }
  }
