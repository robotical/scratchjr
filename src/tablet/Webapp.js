...
export default class Webapp {
    ...
    static cleanassets (ft, fcn) {
        WebappInterface.io_cleanassets(ft); fcn();
    }
    ...
    static setfile (name, str, fcn) {
        var result = WebappInterface.io_setfile({name:name, contents: btoa(str)});
        if (fcn) {
            fcn(result);
        }
    }
    ...
    static sendSjrToShareDialog (fileName, emailSubject, emailBody, shareType, b64data) {
        WebappInterface.sendSjrUsingShareDialog(fileName, emailSubject, emailBody, shareType, b64data);
    }
